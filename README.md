...
Los dibujos de docs/armado/ necesitan además schemdraw 0.23 (`.venv/bin/pip install schemdraw==0.23`). El esquema de la etapa de entrada de ±9 V, con los valores de spice/netlists/input_stage.cir, está en docs/armado/etapa_entrada.png y .pdf, y se rehace con `.venv/bin/python docs/armado/draw_input_stage.py`. El armado de esa misma etapa sobre la protoboard, agujero por agujero, está en docs/armado/etapa_entrada_protoboard.png y sale de `.venv/bin/python docs/armado/draw_input_stage_breadboard.py`, que además imprime la lista de partes y cables. Ese dibujo no se cree solo: antes de dibujar deriva los nodos de la propia protoboard y comprueba que el circuito que queda es el mismo que simula input_stage_split, así que un cable en el agujero equivocado falla ahí y no en la mesa. La alimentación, que es lo que se conecta al final, va aparte en docs/armado/etapa_entrada_alimentacion.png, de `.venv/bin/python docs/armado/draw_input_stage_power.py`. Para armar en la mesa hay además una imagen por paso, con la pieza del paso en color y lo ya puesto en gris: docs/armado/pasos/paso_NN.png y las 21 juntas en docs/armado/etapa_entrada_pasos.pdf, de `.venv/bin/python docs/armado/draw_input_stage_steps.py`, que reusa el trazado y la comprobación contra SPICE del guion anterior, de modo que los pasos no pueden separarse de la placa verificada. Qué hay metido en cada agujerito, fila por fila, y cuáles quedan libres: docs/armado/mapa_de_agujeritos.md, de `.venv/bin/python docs/armado/mapa_agujeritos.py`, generado de los mismos datos. Para qué sirve cada pieza y por qué los cables van donde van: docs/armado/por-que-cada-pieza.md y el resumen en una imagen docs/armado/por_que_cada_pieza.png, de `.venv/bin/python docs/armado/draw_por_que.py`. La revisión con el multímetro previa a las pilas está en docs/armado/prueba_pitido.png, y el símbolo de continuidad que hay que buscar en el aparato en docs/armado/simbolo_pitido.png. Para estudiar la electrónica que hay detrás, en orden y con qué material, docs/aprender-electronica.md.
