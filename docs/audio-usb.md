...
## tono_usb: primer paso de la fase 5 con el ritmo marcado por el USB, sin ADC

firmware/tono_usb.c declara un UAC2 de entrada, 48 kHz fijos, dos canales, 24 bits en subslot de 4 bytes. El canal izquierdo lleva un tono de 1 kHz a -6 dBFS de pico, calculado con una tabla de 48 muestras (48 kHz / 1 kHz), la misma convención de pico que analizador.tono(). El canal derecho va en ceros exactos. Sin ADC todavía: el Pico genera las muestras él mismo, y el USB marca el ritmo pidiendo un paquete de 48 cuadros estéreo cada 1 ms. Probado en el Pico el 2026-09-19: la Mac recibe las muestras idénticas a la tabla (docs/bitacora.md, entrada 32). El firmware limpia el control del buffer IN al cerrarse el stream; sin eso, TinyUSB 0.18.0 en el RP2040 entra en panic al volver a abrirlo.
